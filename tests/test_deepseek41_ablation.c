/* Model-dependent regression: an opt-in scalar optimization must preserve every
...
int main(int argc, char **argv) {
    if (argc != 4) {
        fprintf(stderr, "usage: %s MODEL PROMPT ENV\n", argv[0]);
        return 2;
    }
    unsetenv(argv[3]);
    int rc = 1;
